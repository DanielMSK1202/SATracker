...
import { buildPerformanceProfile, computeFingerprint } from '../_shared/analytics.ts';
import { callGroq } from '../_shared/groq.ts';
import { GLOBAL_ANALYSIS_SYSTEM_PROMPT } from '../_shared/prompts.ts';
import { validateGlobalAnalysis } from '../_shared/validate.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (cache && (!forceRefresh ? cacheIsCurrent : (cacheIsCurrent && withinCooldown))) {
      return jsonResponse({ analysis: cache.analysis, profile: cache.profile, cached: true, analyzedAt: cache.created_at, model: cache.model });
    }
    ...
    return jsonResponse({ analysis, profile, cached: false, analyzedAt, model });
  } catch (err) {
    console.error('ai-analysis unexpected error', err?.message);
    return jsonResponse({ error: 'Something went wrong loading your analysis. Please try again.' }, 500);
  }
});
